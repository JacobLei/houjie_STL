...
inline size_t __deque_buf_size(size_t n, size_t sz)				// 缓冲区大小设计
{
  return n != 0 ? n : (sz < 512 ? size_t(512 / sz) : size_t(1));
}

#ifndef __STL_NON_TYPE_TMPL_PARAM_BUG
template <class T, class Ref, class Ptr, size_t BufSiz>		// deque_iterator 
struct __deque_iterator {
  typedef __deque_iterator<T, T&, T*, BufSiz>             iterator;
  typedef __deque_iterator<T, const T&, const T*, BufSiz> const_iterator;
  static size_t buffer_size() {return __deque_buf_size(BufSiz, sizeof(T)); }
#else /* __STL_NON_TYPE_TMPL_PARAM_BUG */ 								// deque_iterator 
...
struct __deque_iterator {
  ...
  typedef random_access_iterator_tag iterator_category;		// 迭代器分类:随机存取[通过假象实现随机存取]
  ...
  __deque_iterator(T* x, map_pointer y) 			// deque_iterator ctor
    : cur(x), first(*y), last(*y + buffer_size()), node(y) {}
  __deque_iterator() : cur(0), first(0), last(0), node(0) {}	// deque_iterator ctor
  __deque_iterator(const iterator& x)					// deque_iterator ctor
...
public:                         // Basic types
  typedef T value_type;
  typedef value_type* pointer;		// ponter => T*
...
protected:                      // Internal typedefs
  typedef pointer* map_pointer;	// map_pointer => T**
...
public:                         // Insert
  ...
  iterator insert(iterator position, const value_type& x) {
    if (position.cur == start.cur) {		// 如果安插点是deque最前端
      ...
      push_back(x);													// 交给push_bakc()
      iterator tmp = finish;
      --tmp;
      return tmp;
    }
    else {
      return insert_aux(position, x);				// 其他情况，交给insert_aux()
    }
  }
